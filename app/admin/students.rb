          ...
          result.each do |cumulative_payment|
            ...
            cumulative_payment.second.each do |payment|
              tr :class => "#{flip ? 'odd' : 'even'} content" do
                ...
                td number_to_currency(best_in_place_if(current_admin_user.super_admin_or_partner? && payment.due?, payment, :amount, :type => :input, :path => [:admin, payment]), :unit => 'Rs. ', :precision => 0)
                td number_to_currency(best_in_place_if(current_admin_user.super_admin_or_partner? && payment.due?, payment, :discount, :type => :input, :path => [:admin, payment]), :unit => 'Rs. ', :precision => 0)
