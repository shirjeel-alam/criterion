  ...
  show title: :title do
    panel 'Enrollment Details' do
      attributes_table_for enrollment do
        ...
        row(:teacher) { link_to(enrollment.course.teacher.name, admin_teacher_path(enrollment.course.teacher)) rescue nil }
    ...
    panel 'Payments (Books)' do
      table_for enrollment.payments.books.order(:id) do |t|
        t.column(:id) { |payment| link_to(payment.id, admin_payment_path(payment)) }
        t.column(:book) { |payment| link_to(payment.item.name, admin_book_path(payment.item)) }
    ...
    panel 'Payments (Fees)' do
      table_for enrollment.payments.fees.order(:id) do |t|
        t.column(:id) { |payment| link_to(payment.id, admin_payment_path(payment)) }
